  ...
  return (
    <main className="grid min-h-screen lg:grid-cols-2">
      ...
      <aside className="relative hidden overflow-hidden bg-ink-950 p-12 lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="relative">
          ...
          <ul className="mt-6 space-y-3">
            {[t("auth.sideBullet0"), t("auth.sideBullet1"), t("auth.sideBullet2")].map((label) => (
              <li key={label} className="flex items-center gap-2 text-sm text-neutral-400">
        ...
        <p className="relative font-mono text-xs text-neutral-600">{t("auth.builtInLab")} · {brand.domain}</p>
      </aside>
      ...
      <div className="relative flex flex-col justify-center bg-background px-6 py-10">
        ...
        >
          <span aria-hidden>←</span> {t("auth.backToHome")}
        </Link>
        ...
        <div className="mx-auto w-full max-w-sm">
            ...
            <>
              <div className="mt-8 space-y-2.5">
                ...
                >
                  ...
                  {busy === "google" ? t("auth.redirecting") : t("auth.continueGoogle")}
                </button>
                <button
                ...
                >
                  ...
                  {busy === "github" ? t("auth.redirecting") : t("auth.continueGithub")}
                </button>
              </div>
              ...
              <form onSubmit={signInWithEmail} className="space-y-3">
                ...
                <Button type="submit" className="w-full" disabled={status === "loading"}>
                  {status === "loading" ? t("auth.sending") : isSignup ? t("auth.createAccountBtn") : t("auth.sendMagicLink")}
                </Button>
              </form>
          ...
          <p className="mt-4 text-center text-xs text-muted-foreground">
            {t("auth.agreePre")}{" "}
            <Link href="/terms" className="underline hover:text-foreground">{t("auth.terms")}</Link> {t("auth.and")}{" "}
            <Link href="/privacy" className="underline hover:text-foreground">{t("auth.privacyPolicy")}</Link>.
          </p>
        </div>
      </div>
    </main>
